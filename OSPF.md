...
```
                          Area 10              Area 0              Area 40
                     +---------------+    +------------+    +---------------+
                     |               |    |            |    |               |
  [PC1]             se0/0          se0/0 se0/1       se0/0 se0/1           [PC4]
    |     fa0/0      |    1.0.0.0/8  |    | 2.0.0.0/8 |    | 3.0.0.0/8    |    fa0/0
    +------[R1]------+              [R2]--+           [R3]--+             [R4]------+
           |                         |                 |                            |
         192.168                   fa0/0             fa0/0                       223.255
         .10.0/24                    |                 |                         .255.0/24
           |                    126.0.0.0/8       191.255.0.0/8                    |
         [PC1]                   Area 20             Area 30                     [PC4]
                                   |                   |
                                 [PC2]               [PC3]
```
...
```
  [PC1]              [PC2]              [PC3]              [PC4]
 192.168.10.1       126.255.255.1      191.255.255.1      223.255.255.1
    |                   |                   |                   |
    |                   |                   |                   |
  fa0/0              fa0/0              fa0/0              fa0/0
 [R1]----se0/0----se0/0-[R2]-se0/1----se0/0-[R3]-se0/1----se0/0-[R4]
 .10.2   1.1.1.1  1.1.1.2    2.1.1.1  2.1.1.2    3.1.1.1  3.1.1.2   .255.2
          DCE               DCE               DCE
```
...
| 라우터 | 인터페이스 | IP 주소 | Area |
|--------|-----------|---------|------|
| R1 | se0/0 | 1.1.1.1/8 | Area 10 |
| R1 | fa0/0 | 192.168.10.2/24 | Area 10 |
| R2 | se0/0 | 1.1.1.2/8 | Area 10 |
| R2 | se0/1 | 2.1.1.1/8 | Area 0 |
| R2 | fa0/0 | 126.255.255.2/8 | Area 20 |
| R3 | se0/0 | 2.1.1.2/8 | Area 0 |
| R3 | se0/1 | 3.1.1.1/8 | Area 40 |
| R3 | fa0/0 | 191.255.255.2/8 | Area 30 |
| R4 | se0/0 | 3.1.1.2/8 | Area 40 |
| R4 | fa0/0 | 223.255.255.2/24 | Area 40 |
...
|                   |     |                   |     |                   |
|  PC1 --- R1 ------+-----+------ R2 --- R3 --+-----+------ R4 --- PC4 |
|  192.168.10.0/24  |     |    2.0.0.0/8      |     |  223.255.255.0/24 |
|  1.0.0.0/8        |     |                   |     |  3.0.0.0/8        |
+-------------------+     +-------------------+     +-------------------+
                                  |   |
                          +-------+   +-------+
                   ...
                   |             |     |             |
                   |  PC2        |     |  PC3        |
                   | 126.0.0.0/8 |     | 191.255.0.0 |
                   +-------------+     +-------------+

                   R2 = ABR (Area 10 / Area 0 / Area 20)
                   R3 = ABR (Area 0 / Area 40 / Area 30)
```

### 실습 환경
- Cisco Packet Tracer 사용
- 라우터 4대 (R1~R4), PC 4대
- 라우터 간 Serial DCE 케이블, PC-라우터 간 Copper Straight-Through 케이블
...
- 라우터 전원 ON
- 라우터 간 Serial DCE 케이블 연결 (clock rate 설정하는 쪽이 DCE)
...
1.1.1.1        0   FULL/ -   00:00:35   1.1.1.1       Serial0/0
```
...
R1에서 확인하면 아래처럼 보여야 한다:
```
O     1.0.0.0/8        → 같은 Area 경로 (Area 10 내부)
O IA  126.0.0.0/8      → 다른 Area 경로 (Area 20에서 옴)
O IA  191.255.0.0/16   → 다른 Area 경로 (Area 30에서 옴)
O IA  223.255.255.0/24 → 다른 Area 경로 (Area 40에서 옴)
```
...
PC1 > ping 126.255.255.1
PC1 > ping 191.255.255.1
```
...
[R1 fa0/0] 수신
  |
  | 라우팅 테이블: 223.255.255.0/24 → via 1.1.1.2 (O IA)
  v
[R1 se0/0] → 1.0.0.0/8 → [R2 se0/0] 수신
  |
  | 라우팅 테이블: 223.255.255.0/24 → via 2.1.1.2
  v
[R2 se0/1] → 2.0.0.0/8 → [R3 se0/0] 수신
  |
  | 라우팅 테이블: 223.255.255.0/24 → via 3.1.1.2
  v
[R3 se0/1] → 3.0.0.0/8 → [R4 se0/0] 수신
  |
  | 라우팅 테이블: 직접 연결 (fa0/0)
  v
[R4 fa0/0] → PC4 (223.255.255.1)
```
...
```
            ...
            +----------+  +----------+  +----------+  +----------+
            |          |  |          |  |          |  |          |
  s0/0      |    s0/0  s0/1   s0/1  s0/2   s0/2  s0/3   s0/3  |
 [R1]-------+---[R2]---+--+---[R3]--+--+---[R4]---+--+---[R5]  |
  .1        |    .2    .1 |    .2   .1 |    .2    .1 |    .2   |
            |          |  |          |  |          |  |          |
            +----------+  +----------+  +----------+  +----------+
...
```
  [R1]            [R2]            [R3]            [R4]            [R5]
 RID:1.1.1.1    RID:2.2.2.2    RID:3.3.3.3    RID:4.4.4.4    RID:5.5.5.5
   |               |    |         |    |         |    |            |
  s0/0           s0/0  s0/1    s0/1  s0/2     s0/2  s0/3        s0/3
   |               |    |         |    |         |    |            |
   +---192.168.10--+    +--192.168.20--+    +--192.168.30--+    +--192.168.40--+
       ...
       Area 0              Area 1              Area 1              Area 2
                     ...
                     R4 = ABR (Area 1 / Area 2)

              Virtual Link: R2 (2.2.2.2) <---Area 1---> R4 (4.4.4.4)
```

### Virtual Link가 필요한 이유
...
```
  ...
  OSPF 규칙: 모든 Area는 Area 0과 연결되어야 함
  Area 2는 Area 0과 직접 연결이 없음
  → Area 1을 Transit Area로 사용하는 Virtual Link 필요
```
...
show ip route ospf
```
192.168.10.0 경로가 없으면 Virtual Link 설정이 필요한 상태.
...
Virtual Link 설정 흐름:
  ...
  [R2]                                    [R4]
  ABR (Area 0/1)                          ABR (Area 1/2)
    |                                       |
    | area 1 virtual-link 4.4.4.4           | area 1 virtual-link 2.2.2.2
    |                                       |
    +========= Virtual Link (논리) =========+
              Transit Area = Area 1
```
...
이제 R5에서 `show ip route ospf`를 다시 확인하면 192.168.10.0 경로가 O IA로 보인다.
...
```
  ...
  +--------------------------------------------------+
  |                                                  |
  |  [R2] s0/1 -------- [R3] s0/1  s0/2 -------- [R4] s0/2  |
  |       key 1 md5      key 1 md5  key 1 md5      key 1 md5 |
...
| 명령어 | 용도 |
|--------|------|
| `show ip ospf neighbor` | OSPF 이웃 목록 및 상태 확인 |
| `show ip ospf database` | LSDB 확인 (LSA 목록) |
| `show ip ospf interface` | OSPF가 동작하는 인터페이스 확인 |
| `show ip ospf virtual-links` | Virtual Link 상태 확인 |
| `show ip route` | 라우팅 테이블 (O, O IA, O E1/E2 구분) |
| `show ip route ospf` | OSPF 경로만 필터링해서 확인 |
| `clear ip ospf process` | OSPF 프로세스 재시작 (Router ID 변경 시) |
